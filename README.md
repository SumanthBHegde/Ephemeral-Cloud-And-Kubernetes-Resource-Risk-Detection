...
**Resource risk dashboard** — near-real-time risk across ephemeral assets, with a live replay that re-plays an incident and shows it detected ~18h before a daily scan.
![Resource risk dashboard](screenshots/01-dashboard.png)

**Risk findings** — correlated incidents ranked by fused risk score, each tagged with MITRE ATT&CK techniques, event counts, and duration.
![Risk findings](screenshots/02-risk-findings.png)

**Risk analytics** — distributions, trends, and the alert-fatigue funnel (raw → flagged → suppressed → correlated → triaged) across the pipeline.
![Risk analytics](screenshots/03-analytics.png)

**AI risk analyst (LLM triage)** — grounded per-incident triage: likely intent, why-not-benign reasoning, MITRE techniques, key evidence, and recommended guardrails.
![AI risk analyst](screenshots/04-ai-risk-analyst.png)

**Reports** — exportable analyst-ready incident reports.
![Reports](screenshots/05-reports.png)

**Test suite** — every pipeline stage has a dedicated test module; the full suite passes end-to-end (50 tests).
